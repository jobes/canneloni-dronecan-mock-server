...
class HeartbeatPublisher(BasePublisher):
    ...
    """
    def __init__(self, node_id: int, clock: ClockProtocol, interval: float = 1.0, priority: int = DRONECAN_PRIORITY_DEFAULT) -> None:
        ...
        self.can_id: int = build_message_can_id(self.priority, DRONECAN_NODESTATUS_DTID, self.node_id)
    ...
    def process(self, now: float) -> list[tuple[int, bytes]]:
        if now - self.last_send >= self.interval:
            ...
            logger.debug(f"[{uptime_sec:>6}s] TX heartbeat  tid={self.tid:>2} | {can_data.hex()}")
